...
export function DiagonalExactDiagram({ result, L, W, farL, unit, pieceLabel = "Plank", sectionLabel = "layout" }) {
  ...
  return (
    <section style={{ background: COLORS.blueprint, borderRadius: 10, padding: "14px 12px 18px", marginBottom: 12 }}>
      <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: COLORS.chalk, letterSpacing: "0.06em", marginBottom: 6 }}>
        LAYOUT — {L}{unit}{isTrapezoid ? ` – ${effFarL}${unit}` : ""} × {W}{unit} · {isPlank ? "diagonal plank" : "diagonal herringbone"} · {result.totalPlanks} {pieceLabel.toLowerCase()}s
      </div>
      <svg ref={svgRef} viewBox={`0 0 ${svgW} ${svgH}`} width="100%" height="auto" style={{ display: "block" }} preserveAspectRatio="xMidYMid meet" role="img" aria-labelledby="diag-diagram-title">
        <title id="diag-diagram-title">{isPlank ? "Diagonal plank" : "Diagonal herringbone"} layout for a {L}{unit} by {W}{unit} room{isTrapezoid ? ` (far wall ${effFarL}${unit})` : ""}, {result.totalPlanks} planks</title>
