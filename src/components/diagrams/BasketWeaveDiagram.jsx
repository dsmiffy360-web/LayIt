...
export function BasketWeaveDiagram({ result, L, W, unit, pieceLabel = "Plank", sectionLabel = "layout" }) {
  ...
  return (
    <section style={{ background: COLORS.blueprint, borderRadius: 10, padding: "14px 12px 18px", marginBottom: 12 }}>
      <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: COLORS.chalk, letterSpacing: "0.06em", marginBottom: 6 }}>
        LAYOUT — {L}{unit} × {W}{unit} · basket weave · {result.totalPlanks} {pieceLabel.toLowerCase()}s
      </div>
      <svg ref={svgRef} viewBox={`0 0 ${svgW} ${svgH}`} width="100%" height="auto" style={{ display: "block" }} preserveAspectRatio="xMidYMid meet" role="img" aria-labelledby="bw-diagram-title">
        <title id="bw-diagram-title">Basket weave layout for a {L}{unit} by {W}{unit} room, {result.totalPlanks} planks</title>
