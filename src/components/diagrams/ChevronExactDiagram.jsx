...
export function ChevronExactDiagram({ result, L, W, unit, pieceLabel = "Plank", sectionLabel = "layout" }) {
  ...
  return (
    <section style={{ background: COLORS.blueprint, borderRadius: 10, padding: "14px 12px 18px", marginBottom: 12 }}>
      <div style={{ fontFamily: "JetBrains Mono", fontSize: 11, color: COLORS.chalk, letterSpacing: "0.06em", marginBottom: 6 }}>
        LAYOUT — {L}{unit} × {W}{unit} · exact chevron, {result.hbCentered ? "centered" : "corner"} start · {result.totalPlanks} {pieceLabel.toLowerCase()}s
      </div>
      <svg ref={svgRef} viewBox={`0 0 ${svgW} ${svgH}`} width="100%" height="auto" style={{ display: "block" }} preserveAspectRatio="xMidYMid meet" role="img" aria-labelledby="chev-diagram-title">
        <title id="chev-diagram-title">Exact chevron tiling for a {L}{unit} by {W}{unit} room, {result.totalPlanks} planks</title>
