...
export function ChevronExactCutList({ sectionResults, unit, pieceLabel = "Plank", checkedPieces = {}, onBump, onReset }) {
  const decimals = UNIT_DECIMALS[unit];
  let fullCount = 0, sliverCount = 0;
  const simpleTally = new Map(); // rounded length -> count
  const compoundTally = new Map(); // "b×t" -> count

  sectionResults.forEach((sec) => {
    (sec.chevronPieces || []).forEach((p) => {
      const c = classifyChevronCut(p.cutSpec);
    ...
    return (
      ...
      >
        <button
          ...
          disabled={complete}
          style={{ flex: 1, textAlign: "left", background: "none", border: "none", cursor: complete ? "default" : "pointer", padding: 0, fontFamily: "JetBrains Mono" }}
        >
        ...
        {done > 0 && (
          <button
            ...
            aria-label={`Reset count for ${label}`}
  ...
  return (
    <section style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 18, marginBottom: 18 }}>
      <div style={{ fontFamily: "Space Grotesk", fontWeight: 600, fontSize: 15, marginBottom: 4 }}>Chevron cutting list</div>
      <div style={{ fontSize: 12, color: COLORS.sub, marginBottom: 12 }}>
        {totalPlanks} planks total. A wall cut on a 45°-angled plank is itself a 45° cut — the same character as the
        factory point-length miter — so most perimeter pieces reduce to "cut the point-length to X" just like a
        standard board, just shorter. Pieces along the room's other pair of walls need two measurements (a stepped
        cut); a handful of small corner slivers are one-off shapes best measured off the diagram.
      </div>
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
            {simpleRows.map(([len, count]) => (
              <TallyRow
                key={len}
                tallyKey={`chev-simple-${len}`}
                count={count}
                label={`Point-length ${len}${unit} (standard 45° miter)`}
              />
            ))}
          </div>
        </>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
            {compoundRows.map(([key, count]) => {
              ...
              return (
                <TallyRow
                  key={key}
                  tallyKey={`chev-compound-${key}`}
                  count={count}
                  label={`Near edge ${bLen}${unit}, far edge ${tLen}${unit}`}
                  sub="Cut each edge to its own length — the miter steps between them."
                />
              );
            })}
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        >
          <span style={{ fontFamily: "JetBrains Mono", fontSize: 13 }}>Full {pieceLabel.toLowerCase()}s, standard miter only</span>
      ...
      <p style={{ fontSize: 12, color: COLORS.sub, marginTop: 14, marginBottom: 0 }}>
        Start from a chalk line through the room's center (or its longest sightline) and work outward — that keeps the
        pattern symmetrical and pushes the trickiest cuts out to the edges, where they're covered by trim.
      </p>
    </section>
  );
}
